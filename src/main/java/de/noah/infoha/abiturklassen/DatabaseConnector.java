 ...
 * Manipulation einer MySQL-Datenbank. Beim Erzeugen des Objekts wird eine
...
public class DatabaseConnector {
	...
	/**
	 * Ein Objekt vom Typ DatabaseConnector wird erstellt, und eine Verbindung
	 * zur Datenbank wird aufgebaut. Mit den Parametern pIP und pPort werden die
	 * IP-Adresse und die Port-Nummer uebergeben, unter denen die Datenbank mit
	 * Namen pDatabase zu erreichen ist. Mit den Parametern pUsername und
	 * pPassword werden Benutzername und Passwort fuer die Datenbank uebergeben.
	 */
	...
	/**
	 * Der Auftrag schickt den im Parameter pSQLStatement enthaltenen SQL-Befehl
	 * an die Datenbank ab. Handelt es sich bei pSQLStatement um einen
	 * SQL-Befehl, der eine Ergebnismenge liefert, so kann dieses Ergebnis
	 * anschließend mit der Methode getCurrentQueryResult abgerufen werden.
	 */
	public void executeStatement(String pSQLStatement) {
		...
		try {
			...
			if (statement.execute(pSQLStatement)) { // Fall 1: Es gibt ein
													// Ergebnis
	 ...
	 * executeStatement an die Datenbank geschickten SQL-Befehls als Ob-jekt vom
	 * Typ QueryResult zurueck. Wurde bisher kein SQL-Befehl abgeschickt oder
	 * ergab der letzte Aufruf von executeStatement keine Ergebnismenge (z.B.
	...
	/**
	 * Die Anfrage liefert null oder eine Fehlermeldung, die sich jeweils auf
	 * die letzte zuvor ausgefuehrte Datenbankoperation bezieht.
	 */
